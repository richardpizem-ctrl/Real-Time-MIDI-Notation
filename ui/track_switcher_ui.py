    ...
    def update_activity(self, activity_dict):
        ...
        for i in range(self.track_count):
            try:
                ...
                tm.track_activity[i] = level

                if level > tm.peak_hold[i]:
                    tm.peak_hold[i] = level
                else:
                    tm.peak_hold[i] = max(0.0, tm.peak_hold[i] - 0.01)

            except Exception:
                tm.track_activity[i] = 0.0
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if 0 <= my - self.y <= self.button_height:
                ...
                if 0 <= index < self.track_count:
                    ...
                    # MUTE
                    if self.button_height - 22 <= local_y < self.button_height - 12:
